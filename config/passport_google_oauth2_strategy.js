...
passport.UpdateStrategy = async function () {
    ...
    passport.use(new googleStrategy({
        clientID: google_clientID.value,
        clientSecret: google_clientSecret.value,
        callbackURL: google_callbackURL.value,
    },
            ...
            try {
                ...
                if (user) {
                    ...
                    for (let item of sessions)
                        if (!JSON.parse(item.session).passport || user.id == JSON.parse(item.session).passport.user)
            ...
            if (devPermission) {
                if (req.isAuthenticated()) {
                    ...
                    if (permission.users.includes(user.id)) {
                        ...
                        if (daysBool[currentDay] && currentHour >= startHour && currentHour < endHour) {
