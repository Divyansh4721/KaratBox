...
function transformData(item) {
    ...
    let createArr = (item, name, task) => {
        if (item[name] && Array.isArray(item[name]))
            item[name] = item[name].map((value) => task === "date" ? new Date(value) : (value ? new ObjectId(value) : undefined));
    }
    let temp = ["_id", "userGave", "user", "userTake", "dealer", "customer", "prefix", "ornament", "purity", "kaarigar", "stockType", "bill", "createdBy", "deletedBy"];
    for (let i of temp)
        createObject(item, i);
    temp = ["approvedDate", "receivedDate", "createdAt", "updatedAt", "date", "createdDate", "deletedDate"];
    for (let i of temp)
        createDate(item, i);
    temp = ["cart", "bills", "approvals", "opening", "arrival", "edit", "sold", "deleted", "totalApproval", "approvalGive", "approvalTake", "closing", "total", "users", "approveTable"];
    for (let i of temp)
        createArr(item, i, "object");
        ...
        item.stoneTable = item.stoneTable.map((value) => {
            if (value.tounch)
                ...
                return {
                    ...
                    dealerName: value.dealerName ? new ObjectId(value.dealerName) : undefined,
                    _id: value._id ? new ObjectId(value._id) : undefined,
                }
        });
    }
...
module.exports.ImportData = async function (req, res) {
    try {
        ...
        for (const collection of collections) {
            ...
            let data = await fs.readFile('backup/' + collection.name + '.json', 'utf8');
            await fs.unlink('backup/' + collection.name + '.json');
            await db.collection(collection.name).deleteMany({});
            console.log(`Dropped collection: ${collection.name}`);
            let jsonData = JSON.parse(data);
            if (jsonData.length === 0) continue;
            await db.collection(collection.name).insertMany(jsonData.map(transformData));
        }
