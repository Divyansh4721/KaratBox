...
module.exports.stockAddPage = async function (req, res) {
    try {
        ...
        let kaarigarTable = await Kaarigar.find()
            .sort({
                name: 1
            });
        ...
        let stoneDealerTable = await StoneDealer.find()
            .sort({
                name: 1
            });
        let stoneTypeTable = await StoneType.find()
            .sort({
                name: 1
            });
    ...
    try {
        ...
        Stock.uploadedAvatar(req, res, async function (err) {
            ...
            for (let i = 0; i < req.files.length; i++) {
                let dest = path.join(STOCK_PATH, req.body.index + '-' + (i + 1) + ".png");
                await fs.writeFile(dest, req.files[i].buffer, () => {});
                req.files[i].filename = path.join(req.body.index + '-' + (i + 1) + ".png");
            }
            ...
            //Tag
            {
                ...
                let availableArr = indexTable.find(item =>
                    item.prefix && item.prefix.toString() === prefix &&
                    item.ornament && item.ornament.toString() === ornament
                ).available;
            ...
            if (req.body.stoneType) {
                for (let i = 0; i < req.body.stoneType.length; i++) {
                    let temp = {
                        ...
                        dealerName: req.body.stoneDealer[i] ? req.body.stoneDealer[i] : undefined,
                    }
...
module.exports.stockEditForm = async function (req, res) {
    try {
        ...
        Stock.uploadedAvatar(req, res, async function (err) {
            ...
            let STOCK_PATH = path.join(env.assetPath);
            let fs = require("fs");
            for (let i = 0; await fs.existsSync(path.join(STOCK_PATH, req.body.index + '-' + (i + 1) + ".png")); i++) {
                await fs.unlinkSync(path.join(STOCK_PATH, req.body.index + '-' + (i + 1) + ".png"));
            }
            let tempStock = await Stock.findOne({
                index: req.body.index
            });
            for (let i = 0; i < tempStock.stockImage.length; i++) {
                if (await fs.existsSync(path.join(STOCK_PATH, tempStock.stockImage[i].fileName))) {
                    await fs.unlinkSync(path.join(STOCK_PATH, tempStock.stockImage[i].fileName));
                }
            }
            for (let i = 0; i < req.files.length; i++) {
                let dest = path.join(STOCK_PATH, req.body.index + '-' + (i + 1) + ".png");
            ...
            if (((tempStock.grossWt * 1).toFixed(3)).toString() !== ((grossWt * 1).toFixed(3)).toString()) {
                AddRemark(tempStock, "grossWt", tempStock.grossWt, grossWt);
                tempStock.grossWt = grossWt;
            }
            if (((tempStock.netWt * 1).toFixed(3)).toString() !== ((netWt * 1).toFixed(3)).toString()) {
                AddRemark(tempStock, "netWt", tempStock.netWt, netWt);
                tempStock.netWt = netWt;
            }
            if (((tempStock.stoneWt * 1).toFixed(3)).toString() !== ((stoneWt * 1).toFixed(3)).toString()) {
            ...
            if (tempStock.stockType.toString() !== stockType.toString()) {
                ...
                AddRemark(tempStock, "stockType", tempValStock.stockType.name, temp.name);
            ...
            for (let i = 0; isSame && i < stoneTable.length; i++) {
                if (
                    ...
                    (Number(tempStock.stoneTable[i].gmWeight) !== Number(stoneTable[i].gmWeight)) ||
                    (Number(tempStock.stoneTable[i].sellRate || '') !== Number(stoneTable[i].sellRate || '')) ||
                    (Number(tempStock.stoneTable[i].purchaseRate || '') !== Number(stoneTable[i].purchaseRate || '')) ||
                    (String(tempStock.stoneTable[i].dealerName || '') !== String(stoneTable[i].dealerName || ''))
                ) {
                    isSame = false;
                }
                if (String(tempStock.stoneTable[i].type) !== String(stoneTable[i].type)) {
                ...
                if (Number(tempStock.stoneTable[i].ctWeight) !== Number(stoneTable[i].ctWeight)) {
                    AddRemark(tempStock, "StoneTable ctWeight", tempStock.stoneTable[i].ctWeight, stoneTable[i].ctWeight);
                }
                if (Number(tempStock.stoneTable[i].gmWeight) !== Number(stoneTable[i].gmWeight)) {
                    AddRemark(tempStock, "StoneTable gmWeight", tempStock.stoneTable[i].gmWeight, stoneTable[i].gmWeight);
                }
                if (Number(tempStock.stoneTable[i].sellRate || '') !== Number(stoneTable[i].sellRate || '')) {
                    AddRemark(tempStock, "StoneTable sellRate", tempStock.stoneTable[i].sellRate, stoneTable[i].sellRate);
                }
                if (Number(tempStock.stoneTable[i].purchaseRate || '') !== Number(stoneTable[i].purchaseRate || '')) {
                    AddRemark(tempStock, "StoneTable purchaseRate", tempStock.stoneTable[i].purchaseRate, stoneTable[i].purchaseRate);
                }
                if (String(tempStock.stoneTable[i].dealerName || '') !== String(stoneTable[i].dealerName || '')) {
                    let temp1 = tempStock.stoneTable[i].dealerName ? await StoneDealer.findById(tempStock.stoneTable[i].dealerName) : "";
                    let temp2 = stoneTable[i].dealerName ? await StoneDealer.findById(stoneTable[i].dealerName) : "";
                    AddRemark(tempStock, "StoneTable dealerName", temp1 ? temp1.name : "", temp2 ? temp2.name : "");
                }
            }
            if (!isSame) {
            ...
            // }
            if ((tempStock.kaarigar && !kaarigar) ||
                (!tempStock.kaarigar && kaarigar) ||
                ((tempStock.kaarigar && kaarigar) &&
                    (tempStock.kaarigar.toString() !== kaarigar.toString()))) {
                let temp = "";
                if (kaarigar)
                    temp = await Kaarigar.findById(kaarigar);
                AddRemark(tempStock, "kaarigar", tempValStock.kaarigar ? tempValStock.kaarigar.name : "", temp ? temp.name : "");
                tempStock.kaarigar = kaarigar ? kaarigar : undefined;
            }
    ...
    try {
        ...
        for (let i = 0; i < stockTable.length; i++) {
            stockTable[i].updatedTable.push({
                user: req.user.id,
                date: new Date(),
                remark: "stockType" + " old: " + tempStockTable[i].stockType.name + " new: " + newStockType.name
            });
