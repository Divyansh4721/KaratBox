...
async function calculatePrice(item) {
  ...
  const metalCost =
    (retailMultiplier * item.netWt * goldRate * (100 + +wastage)) / 10000;
  const stonesCost = item.stoneTable.reduce(
    (sum, { ctWeight = 1, sellRate }) => sum + ctWeight * +sellRate,
    0
  );
...
module.exports.DimossWebsite = async function (req, res) {
  try {
    let stockTable = await Stock.find({
      prefix: {
        $in: [
          "66c4c3b4d6873b8ea50b5893",
          "66c4c3b9d6873b8ea50b6555",
          "67c2c48cd3604e039791483a"
        ]
      }
