...
module.exports.convertTime = function (date) {
    ...
    let result = today.getDate() + ' ' + month[today.getMonth()] + ' ' + today.getFullYear() + ' ( ' +
...
module.exports.calculatePrice = async function (item) {
    ...
    item.sellingPrice = item.purity.retailMultiplier * item.netWt * goldRate * (100 + (item.purity.wastage * 1)) / 100.0 / 100.0;
    ...
    for (let i = 0; i < temp.length; i++) {
        ...
        if (temp[i].StoneType1) {
            x = {
                ...
                sellRate: ((temp[i].StoneRate1 * 1) / (temp[i].StoneWeight1 * 1)).toFixed(0),
                purchaseRate: 0,
                dealerName: null,
            }
            stoneTable.push(x);
        ...
        if (temp[i].StoneType2) {
            x = {
                ...
                sellRate: ((temp[i].StoneRate2 * 1) / (temp[i].StoneWeight2 * 1)).toFixed(0),
                purchaseRate: 0,
                dealerName: null,
            }
            stoneTable.push(x);
        ...
        if (temp[i].StoneType3) {
            x = {
                ...
                sellRate: ((temp[i].StoneRate3 * 1) / (temp[i].StoneWeight3 * 1)).toFixed(0),
                purchaseRate: 0,
                dealerName: null,
            }
            stoneTable.push(x);
