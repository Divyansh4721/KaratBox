...
module.exports.addToCart = async function (req, res) {
    try {
        ...
        user.cart = Array.from(new Set(user.cart.map(item => item.toString()))).map(id => new ObjectId(id));
        await user.save();
        if (req.xhr) {
...
module.exports.soldRetailPage = async function (req, res) {
    try {
        ...
        for (let i = 0; i < customerTable.length; i++) {
            customerTable[i].searchStr = customerTable[i].name + " " +
...
module.exports.soldRetailForm = async function (req, res) {
    try {
        ...
        for (let i = 0; i < stockTable.length; i++) {
            ...
            index.available = index.available.filter(obj => obj != stockTable[i].tag)
            await index.save();
        }
...
module.exports.approvalRecvForm = async function (req, res) {
    try {
        let approval = await Approval.findById(req.body.approval)
            .populate('cart');
